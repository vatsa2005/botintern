...
async function loop(page, prompt = "") {
    ...
    const result = await fix([fileTree, sourceCode, currentYaml, prompt]);
    const updates = parseAiResponse(result);

    if (!updates || updates.length === 0) {
        console.log(chalk.red("❌ AI returned unparseable code. Skipping..."));
    } else {
        applyFileUpdates(updates);

        console.log(chalk.green('✨ Fixes applied. Retrying build...'));
    }
