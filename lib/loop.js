...
async function loop(page, prompt = "", iteration = 1, maxIterations = 5) {
    ...
    if (prompt && iteration === 1) {
        spinner = ora(chalk.yellow('👀 Generating yaml test cases...')).start();
        try {
    ...
    spinner = ora(chalk.yellow('🔨 Building project...')).start();
            ...
            if (error) {
                ...
                try {
                    ...
                    if (fs.existsSync(absolutePath)) {
                        spinner = ora(chalk.cyan('🤖 AI fixing the code...')).start();
                ...
                spinner.succeed(chalk.green('✅ Build Passed!'));
                spinner = null;
    ...
    if (testResult.success) {
        ...
        try {
            spinner = ora(chalk.cyan('🔧 Generating test-driven fixes...')).start();
            ...
            spinner.succeed(chalk.green('✅ Fix suggestions generated'));

            // Parse and apply the fixes
            spinner = ora(chalk.blue('📝 Parsing AI response...')).start();
            const updates = parseAiResponse(aiResponse);

            if (!updates || updates.length === 0) {
                spinner.fail(chalk.red('❌ No valid file updates found in AI response'));
                spinner = null;
                console.log(chalk.yellow('💡 The AI may have misunderstood. Retrying...'));
                return loop(page, "", iteration + 1, maxIterations);
            }

            spinner.succeed(chalk.green(`✅ Found ${updates.length} file(s) to update`));
            spinner = null;
